...
export function getSystemPrompt(
  ...
  parts.push(getEnvironmentSection(config));

  if (tools) {
    parts.push(getToolGuidelinesSection(tools));
  }
  ...
  if (config.developerInstructions) {
    parts.push(getDeveloperInstructionsSection(config.developerInstructions));
  }

  if (config.userInstructions) {
    parts.push(getUserInstructionsSection(config.userInstructions));
  }
...
function getEnvironmentSection(config: Config): string {
  const now = new Date();
...
2. **Plan:** Build a coherent and grounded (based on the understanding in step 1) plan for how you intend to resolve the user's task. For complex tasks with dependencies or multiple sequential steps, use the \`plan\` tool to create a structured plan. For simpler tasks, use the \`todos\` tool to track your progress. Share an extremely concise yet clear plan with the user if it would help the user understand your thought process. As part of the plan, you should use an iterative development process that includes writing unit tests to verify your changes.
...
- **Plan Mode:** For complex tasks that require multiple steps with dependencies, use the \`plan\` tool to create a structured plan. The plan tool allows you to:
  - Create a detailed plan with steps
  - Track step execution status (pending, in_progress, completed, failed)
  - Manage step dependencies
  - Execute steps in order
  - View progress statistics
  Use \`plan\` when the task is complex enough to benefit from structured planning, especially when steps have dependencies or need to be executed in a specific order. For simple tasks, use \`todos\` instead.
...
function getDeveloperInstructionsSection(instructions: string): string {
...
function getUserInstructionsSection(instructions: string): string {
...
function getToolGuidelinesSection(tools: Tool[]): string {
  const regularTools = tools.filter((t) => !t.name.startsWith("subagent_"));
  const subagentTools = tools.filter((t) => t.name.startsWith("subagent_"));
...
`;
  ...
  for (const tool of regularTools) {
    ...
    guidelines += `- **${tool.name}**: ${description}\n`;
  }
...
4. **Task Management**:
   - Use \`todos\` to track simple multi-step tasks
   - Use \`plan\` for complex tasks with dependencies and structured planning
   - Mark tasks/steps as completed as you finish them
...
export function createLoopBreakerPrompt(loopDescription: string): string {
  return `
